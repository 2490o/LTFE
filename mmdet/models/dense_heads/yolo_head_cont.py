...
from mmdet.models.losses.oadg.contrastive_loss import supcontrast_yolo
...
class YOLOV3HeadCont(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(self,
        ...
        self.loss_cont = supcontrast_yolo
    ...
    def _init_layers(self):
        ...
        for i in range(self.num_levels):
            ...
            self.convs_cont.append(conv_cont)
    ...
    def loss_single(self, pred_map, cont_map, target_map, neg_map):
        ...
        loss_cls = self.loss_cls(pred_label[[0, 1]], target_label[[0, 1]], weight=pos_mask[[0, 1]])
        loss_conf = self.loss_conf(pred_conf[[0, 1]], target_conf[[0, 1]], weight=pos_and_neg_mask[[0, 1]])
        loss_xy = self.loss_xy(pred_xy[[0, 1]], target_xy[[0, 1]], weight=pos_mask[[0, 1]])
        ...
        if self.cont_weight == 0:
            ...
            target_label = target_label.reshape(-1, 8)
            target_label = torch.argmax(target_label, dim=1).reshape(-1, 1)

            targets = torch.ones_like(pos_mask, dtype=target_label.dtype) * 8  # init target with background label
            pos_inds, _ = (pos_mask != 0).nonzero(as_tuple=True)
            neg_inds, _ = (neg_mask != 0).nonzero(as_tuple=True)

            neg_inds = torch.randint(len(neg_inds), (min(2000, len(neg_inds)),)).cuda()
            pos_neg_inds = torch.cat((pos_inds, neg_inds), dim=0)

            targets[pos_inds, :] = target_label[pos_inds, :]
            targets = targets[pos_neg_inds, :]
            cont_feats = cont_feats[pos_neg_inds, :]

            loss_cont = self.cont_weight * self.loss_cont(cont_feats, targets, temper=self.cont_temperature,
                                                          min_samples=10)
